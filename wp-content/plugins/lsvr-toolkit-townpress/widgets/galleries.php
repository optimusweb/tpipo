				...
				<?php if ( ! empty( $galleries ) ) : ?>
					...
					<?php foreach ( $galleries as $gallery ) : ?>
						...
						<?php if ( isset( $show_thumb ) && $show_thumb === 'on' ) : ?>
							<?php $gallery_arr = get_post_meta( $gallery->ID,'meta_gallery_images', true ); ?>
							<?php if ( has_post_thumbnail( $gallery->ID ) ) : ?>
								<?php $thumb_data = lsvr_get_image_data( get_post_thumbnail_id( $gallery->ID ) ); ?>
						...
						<?php $item_class = $current_id === $gallery->ID ? ' m-active' : ''; ?>
						<?php $item_class .= $thumb_data ? ' m-has-thumb' : ''; ?>
						<li class="<?php echo esc_attr( trim( $item_class ) ); ?>">
							<div class="gallery-inner">

								<?php if ( $thumb_data ) : ?>
									<a href="<?php echo get_the_permalink( $gallery->ID ); ?>" class="gallery-image"><img src="<?php echo esc_url( $thumb_data['thumbnail'] ); ?>" alt="<?php echo esc_attr( $thumb_data['alt'] ); ?>"></a>
								<?php endif; ?>

								<h4 class="gallery-title"><a href="<?php echo get_the_permalink( $gallery->ID ); ?>"><?php echo $gallery->post_title; ?></a></h4>

								<?php if ( isset( $show_date ) && $show_date === 'on' ) : ?>
								<div class="gallery-date"><?php echo get_the_date( get_option( 'date_format' ), $gallery->ID ); ?></div>
								<?php endif; ?>

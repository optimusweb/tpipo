...
class Lsvr_Event_Categories_Widget extends WP_Widget {

    public function __construct() {
        $widget_ops = array( 'classname' => 'lsvr-event-categories', 'description' => __( 'List event categories.', 'lsvrtoolkit' ) );
        parent::__construct( 'lsvr_event_categories_widget', __( 'LSVR Event Categories', 'lsvrtoolkit' ), $widget_ops );
    }

    function form( $instance ) {

        $instance = wp_parse_args( (array) $instance, array( 'title' => __( 'Event Categories', 'lsvrtoolkit' ), 'show_count' => 'on' ) );
            ...
            <div class="widget-content">
				...
				if ( $loop->have_posts() ) {
					while ( $loop->have_posts() ) {
						$loop->the_post();
						$terms = wp_get_object_terms( get_the_id(), 'lsvreventcat' );
						foreach ( $terms as $term ) {
							if ( array_key_exists( $term->term_id, $categories_arr ) ) {
								...
								if ( function_exists( 'lsvr_get_term_parents' ) ) {
									$term_parents = lsvr_get_term_parents( $term->term_id, 'lsvreventcat' );
				...
				<?php wp_list_categories( array(
					'taxonomy' => 'lsvreventcat',
					'show_count' => $show_count,
					'title_li' => '',
					'include' => $included_categories_id,
					'walker' => new Lsvr_Event_Categories_Walker,
...
if ( ! class_exists( 'Lsvr_Event_Categories_Walker' ) ) {
class Lsvr_Event_Categories_Walker extends Walker_Category {
    ...
    function start_el( &$output, $item, $depth=0, $args=array(), $current_object_id = 0 ) {
        ...
        if ( $is_archive ) {
            $output .= '<a href="' . esc_url( add_query_arg( array( 'archive' => 'true' ), get_term_link( $item, 'lsvreventcat' ) ) ) . '">' . esc_attr( $item->name ) . '</a>';
        }
        else {
            $output .= '<a href="' . esc_url( get_term_link( $item, 'lsvreventcat' ) ) . '">' . esc_attr( $item->name ) . '</a>';
        }
